...
function RoomFormContent({ room, open, onOpenChange }: RoomFormDialogProps) {
  ...
  const createMutation = usePostApiRooms({
    mutation: {
      ...
      onError: (err) =>
        toast.error(
          err instanceof Error ? err.message : t("rooms.toast.createError")
        ),
    },
  });
  ...
  const updateMutation = usePutApiRoomsId({
    mutation: {
      ...
      onError: (err) =>
        toast.error(
          err instanceof Error ? err.message : t("rooms.toast.updateError")
        ),
    },
  });
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid gap-1.5">
            ...
            <Select value={locationId} onValueChange={setLocationId} required>
              <SelectTrigger>
                <SelectValue
                  placeholder={t("rooms.form.locationPlaceholder")}
                />
              </SelectTrigger>
              <SelectContent>
                {locations.map((loc) => (
