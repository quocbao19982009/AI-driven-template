...
| **Authentication & Authorization** | `feature-spec-auth.md` | JWT auth + role-based authorization plan. Roles: Admin, Manager, Employee. `POST /api/auth/login|refresh|logout`. User management for Admins. Role-gated write actions on all existing endpoints. | Plan only |
