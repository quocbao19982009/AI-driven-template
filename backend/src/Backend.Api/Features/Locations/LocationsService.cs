...
public class LocationsService : ILocationsService
{
    ...
    public async Task<LocationDto> CreateAsync(CreateLocationRequest request, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Created Location {LocationId}", created.Id);
    ...
    public async Task DeleteAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        if (await _repository.HasRoomsAsync(id, cancellationToken))
            throw new Common.Exceptions.ValidationException(["Location cannot be deleted because it has rooms assigned to it."]);

        await _repository.DeleteAsync(entity, cancellationToken);
        _logger.LogInformation("Deleted Location {LocationId}", id);
    }

    private static LocationDto MapToDto(Location entity) => new(entity.Id, entity.Name, entity.CreatedAt);
